...
window.CodeMirror = (function () {
    ...
    function updateDisplayInner(cm, changes, viewPort) {
        ...
        var intact = changes === true ? [] :
            computeIntact([
                {from: display.showingFrom, to: display.showingTo}
            ], changes);
    ...
    function registerEventHandlers(cm) {
        ...
        if (!gecko) on(d.scroller, "contextmenu", function (e) {
            onContextMenu(cm, e);
        });
        ...
        function reFocus() {
            if (cm.view.focused) setTimeout(bind(focusInput, cm), 0);
        }
    ...
    function onKeyPress(e) {
        ...
        if (this.options.electricChars && this.view.mode.electricChars &&
            ...
            setTimeout(operation(cm, function () {
                indentLine(cm, cm.view.sel.to.line, "smart");
            }), 75);
    ...
    function onBlur(cm) {
        ...
        setTimeout(function () {
            if (!cm.view.focused) cm.view.sel.shift = false;
        }, 150);
    }
    ...
    function updateDocNoUndo(cm, from, to, lines, selUpdate, origin) {
        ...
        if (selUpdate && typeof selUpdate != "string") {
            if (selUpdate.from) {
                newSelFrom = selUpdate.from;
                newSelTo = selUpdate.to;
            }
    ...
    CodeMirror.prototype = {
        ...
        getSelection: function (lineSep) {
            return this.getRange(this.view.sel.from, this.view.sel.to, lineSep);
        },
        ...
        isClean: function () {
            return this.view.history.dirtyCounter == 0;
        },
        ...
        clipPos: function (pos) {
            return clipPos(this.view.doc, pos);
        },
        ...
        somethingSelected: function () {
            return !posEq(this.view.sel.from, this.view.sel.to);
        },
    ...
    var commands = CodeMirror.commands = {
        ...
        goDocStart: function (cm) {
            cm.extendSelection({line: 0, ch: 0});
        },
        goDocEnd: function (cm) {
            cm.extendSelection({line: cm.lineCount() - 1});
        },
    ...
    function addLineWidget(cm, handle, node, options) {
        ...
        changeLine(cm, handle, function (line) {
            ...
            if (!lineIsHidden(line) || widget.showIfHidden) {
                ...
                if (aboveVisible)
                    setTimeout(function () {
                        cm.display.scroller.scrollTop += widget.height;
                    });
            }
            return true;
        });
    ...
    function highlightLine(cm, line, state) {
        ...
        runMode(cm, line.text, cm.view.mode, state, function (txt, style) {
            st.push(txt, style);
        });
